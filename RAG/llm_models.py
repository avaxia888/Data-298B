...
class LLMService:
    ...
    def __init__(self):
        ...
        # Use trust_remote_code=False to avoid authentication issues
        try:
            self.embed_model = SentenceTransformer("all-MiniLM-L6-v2", trust_remote_code=False)
        except:
            # Fallback to a different model if needed
            self.embed_model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2", trust_remote_code=False)
