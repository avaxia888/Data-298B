...
def main():
    ...
    with st.sidebar:
        ...
        model_category = st.radio(
            "Select Category",
            ["Finetuned", "RAG (Base)", "RAG + Finetuned"],
            index=0 if finetuned else 1,
        )
        if model_category == "Finetuned":
            pool = finetuned
        elif model_category == "RAG (Base)":
            pool = rag_models
        else:  # RAG + Finetuned
            pool = finetuned
        ...
        is_rag_request = model_category != "Finetuned"

        st.divider()
        temperature = st.slider("Temperature", 0.0, 1.5, 0.7, 0.1)
        max_new_tokens = (
            st.slider("Max new tokens", 16, 4000, 256, 16) if model_category == "Finetuned" else None
        )
        memory_length = (
            st.slider("Conversation memory", 1, 10, 5, 1) if model_category.startswith("RAG") else 0
        )
