...
# Add parent directory to path to import rag_pipeline.py
...
try:
    ...
    RAGPipeline = None  # Pipeline optional; handled at runtime
...
class LLMClient:
        ...
        *,
        ...
        if mode == "rag":
            if RAGPipeline is None:
                raise RuntimeError("rag_pipeline module not found – ensure it's in project root.")
            return self._generate_rag(prompt if prompt else (messages[0]['content'] if messages else ""))
        if mode == "huggingface":
    ...
    def _generate_rag(self, query: str) -> str:
        """Generate answer locally using the RAG pipeline."""
        rag = getattr(self, "_rag_instance", None)
        if rag is None:
            rag = RAGPipeline()
            self._rag_instance = rag  # cache
